...
mod tests {
    ...
    #[test]
    fn test_compile_signatures() {
        let (answer_sigs, guess_sigs) = Packer::compile_signatures(TEST_ANSWERS, TEST_GUESSES);

        assert_eq!(answer_sigs.len(), 2);
        assert_eq!(guess_sigs.len(), 4);

        // Should be sorted and unique
        assert!(answer_sigs.windows(2).all(|w| w[0] <= w[1]));
        assert!(guess_sigs.windows(2).all(|w| w[0] <= w[1]));
    }

    #[test]
    fn test_find_triples_for_answer() {
        ...
        let triples = Packer::find_triples_for_answer(answer, guesses);

        // All triples should be disjoint with the answer
        for triple in &triples {
            assert!(answer.disjoint(triple.mask));
        }
    }

    #[test]
    fn test_packing_creation() {
        let answer = Signature::new("slate");
        let guesses = [
            Signature::new("brick"),
            Signature::new("jumpy"),
        ...
        assert_eq!(packing.answer(), &answer);
        assert_eq!(packing.guesses(), &guesses);
    }
}
